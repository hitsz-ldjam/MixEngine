...
namespace Mix {
    ...
    void Gltf::processNode(const tinygltf::Model& _gltfModel, const tinygltf::Node& _gltfNode, Model::Node& _parentNode) const {
        ...
        if (_gltfNode.matrix.size() == 16) {
            Matrix4::Decompose(MakeMatrix4(_gltfNode.matrix.data()),
                                     translation, rotation, scale);
        }
        else {
            if (_gltfNode.translation.size() == 3) {
                translation = MakeVector3(_gltfNode.translation.data());
            }

            if (_gltfNode.rotation.size() == 4) {
                rotation = Quaternion(_gltfNode.rotation[3], _gltfNode.rotation[0], _gltfNode.rotation[1], _gltfNode.rotation[2]);
            }

            if (_gltfNode.scale.size() == 3) {
                scale = MakeVector3(_gltfNode.scale.data());
            }
        }
    ...
    void Gltf::PopulateMeshAttributeData(MixMeshData& _meshData, const tinygltf::Model& _gltfModel, const tinygltf::Mesh& _gltfMesh) {
        ...
        for (uint32_t i = 0; i < _gltfMesh.primitives.size(); ++i) {
            ...
            if (_gltfMesh.primitives[i].attributes.count(GetGltfAttributeString(GltfAttribute::TANGENT))) {
                ...
                auto p = reinterpret_cast<const Vector4f*>(ptr);
                std::transform(p, p + count, reinterpret_cast<Vector3f*>(_meshData.tangents->data()), [](const Vector4f& _v) {return Vector3f(_v); });
            }
    ...
    void Gltf::ConvertTransformToLeftHanded(Vector3f& _translation, Quaternion& _rotation, Vector3f& _scale) {
        _translation = Vector3f(-_translation.x, _translation.y, _translation.z);
        _rotation = Quaternion(-_rotation.w, -_rotation.x, _rotation.y, _rotation.z);
    }
    ...
    VertexAttribute Gltf::GetVertexAttribute(GltfAttribute _gltfAttribute) {
        ...
        default:return VertexAttribute::None;
        }
    }
