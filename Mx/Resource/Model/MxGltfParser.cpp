...
namespace Mix {
    namespace Resource {
        Object* GltfParser::load(const std::filesystem::path& _path, const ResourceType _type) {
        ...
        Object* GltfParser::load(const std::filesystem::path& _path, const std::string& _ext) {
            Utils::GltfLoader::ModelData modelData;
            bool success = false;
            if (_ext == "glb")
                success = mLoader.loadFromGlb(modelData, _path, _path.filename().string());
        ...
        Object* GltfParser::parse(const Guid& _key) {
