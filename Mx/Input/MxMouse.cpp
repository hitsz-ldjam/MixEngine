...
namespace Mix {
	...
	float Mouse::getAxis(AxisCode _axis) {
		if (_axis == AxisCode::Mouse_X) return mPosState.x;
		if (_axis == AxisCode::Mouse_Y) return mPosState.x;
		if (_axis == AxisCode::Mouse_WheelX) return mWheelState.x;
		if (_axis == AxisCode::Mouse_WheelY) return mWheelState.y;
	}
	...
	void Mouse::nextFrame() {
		...
		mDeltaPosState = Math::Vector2i::Zero;
		mWheelState = Math::Vector2i::Zero;
	}
