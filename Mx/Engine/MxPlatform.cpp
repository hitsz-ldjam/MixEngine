...
namespace Mix {
    ...
    bool Platform::sInitialized = false;
    bool Platform::sCursorVisible = false;
    bool Platform::sRelativeMouseMode = false;
    std::string Platform::sPlantformName = "Unknown";

    Event<void(const PFMouseMoveEventData&)>		Platform::MouseMoveEvent;
    Event<void(const PFMouseButtonEventData&)>		Platform::MouseButtonEvent;
    Event<void(const PFMouseWheelEventData&)>		Platform::MouseWheelEvent;
    Event<void(const PFKeyboardEventData&)>			Platform::KeyboardEvent;
    Event<void(const PFGamepadAxisEventData&)>		Platform::GamepadAxisEvent;
    Event<void(const PFGamepadButtonEventData&)>	Platform::GamepadButtonEvent;
    Event<void(const PFGamepadDeviceEventData&)>	Platform::GamepadDeviceEvent;
    Event<void()>									Platform::QuitEvent;
    ...
    const std::string& Platform::GetPlantformName() {
        return sPlantformName;
    }

    void Platform::SetCursorPositionGlobal(const Vector2i& _screenPos) {
    ...
    bool Platform::CopyToClipboard(const std::string& _string) {
        return SDL_SetClipboardText(_string.c_str()) == 0;
    }

    bool Platform::HasClipText() {
        return SDL_HasClipboardText();
    }

    void Platform::SetRelativeMouseMode(bool _mode) {
        sRelativeMouseMode = _mode;
        SDL_SetRelativeMouseMode(_mode ? SDL_TRUE : SDL_FALSE);
    }

    bool Platform::GetRelativeMouseMode() {
        return sRelativeMouseMode;
    }

    const std::string Platform::CopyFromClipboard() {
    ...
    void Platform::PushQuitEvent() {
        SDL_Event quit;
        quit.type = SDL_QUIT;
        SDL_PushEvent(&quit);
    }

    void Platform::Sleep(uint32_t _duration) {
